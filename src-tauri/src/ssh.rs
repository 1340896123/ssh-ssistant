...
const JUMP_HOST_TIMEOUT: Duration = Duration::from_secs(30);
const LOCAL_FORWARD_TIMEOUT: Duration = Duration::from_secs(10);
const CONNECTION_RETRY_BASE_DELAY: Duration = Duration::from_millis(500);
const CONNECTION_RETRY_MAX_ATTEMPTS: u32 = 3;
...
fn ssh2_retry<F, T>(mut f: F) -> Result<T, ssh2::Error>
where
...
fn establish_connection_with_retry(config: &SshConnConfig) -> Result<ManagedSession, String> {
...
pub async fn test_connection(config: SshConnConfig) -> Result<String, String> {
    execute_ssh_operation(move || {
        let session = establish_connection_with_retry(&config)?;
...
pub async fn read_remote_file(
    ...
    execute_ssh_operation(move || {
        ...
        let mut file = ssh2_retry(|| sftp.open(std::path::Path::new(&path)))
            .map_err(|e| e.to_string())?;
        let mut buf = Vec::new();
...
pub async fn create_directory(
    ...
    execute_ssh_operation(move || {
        ...
        match ssh2_retry(|| sftp.mkdir(std::path::Path::new(&path), 0o755))
            .map_err(|e| e.to_string()) {
            Ok(_) => Ok(()),
            Err(e) => {
...
pub async fn create_file(
    ...
    execute_ssh_operation(move || {
        ...
        match ssh2_retry(|| sftp.create(std::path::Path::new(&path)))
            .map_err(|e| e.to_string()) {
            Ok(_) => Ok(()),
            Err(e) => {
                let error_msg = e.to_string();
...
pub async fn delete_item(
    ...
    execute_ssh_operation(move || {
        ...
        if is_dir {
            ...
            ssh2_retry(|| sftp.unlink(std::path::Path::new(&path)))
                .map_err(|e| e.to_string())?;
        }
        Ok(())
    }).await
...
pub async fn download_file(
    ...
    execute_ssh_operation(move || {
        ...
        let sftp = ssh2_retry(|| sess.sftp()).map_err(|e| e.to_string())?;
        let mut remote_file =
            ssh2_retry(|| sftp.open(std::path::Path::new(&remote_path)))
                .map_err(|e| e.to_string())?;
...
fn upload_recursive(
    ...
    if local_path.is_dir() {
        ...
        let _ = ssh2_retry(|| sftp.mkdir(std::path::Path::new(remote_path), 0o755));
        ...
        let mut local_file = std::fs::File::open(local_path)
            .map_err(|e| e.to_string())?;
        let mut remote_file = ssh2_retry(|| sftp.create(std::path::Path::new(remote_path)))
            .map_err(|e| e.to_string())?;
...
fn upload_recursive_progress(
    ...
    if local_path.is_dir() {
        ...
        if offset > 0 {
            ...
            remote_file = ssh2_retry(|| sftp.create(std::path::Path::new(remote_path)))
                .map_err(|e| e.to_string())?;
        }
