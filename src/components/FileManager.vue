...
const selectedTargetLabel = computed(() => {
    ...
    if (selectedCount.value === 1) {
        if (isTreeMode.value) {
            const [path] = Array.from(selectedTreePaths.value);
            return path || currentLocationLabel.value;
        }
        const [name] = Array.from(selectedFiles.value);
        return name ? pathUtils.value.join(currentPath.value, name) : currentLocationLabel.value;
    }
    ...
    <div ref="containerRef" tabindex="0" class="flex h-full min-h-0 min-w-0 flex-col bg-bg-primary p-2 text-text-primary outline-none" @click="handleContainerClick">
        ...
        <div ref="fileListScrollRef" class="min-h-0 flex-1 overflow-y-auto border border-subtle rounded bg-bg-primary/80 backdrop-blur-sm"
            ...
            <div v-else-if="viewMode === 'flat' && isLoadingNextPage" class="p-3 text-center text-text-tertiary text-xs">
                {{ t('fileManager.hints.loadingMore') }}
            </div>
        </div>
