    ...
    <div class="bg-gray-800 p-6 rounded shadow-lg w-[500px] text-white max-h-[90vh] overflow-y-auto">
      ...
      <div class="space-y-4">
        <div>
          ...
          <input v-model="form.name" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="My Server" />
        </div>
        <div class="grid grid-cols-4 gap-4">
            <div class="col-span-3">
              <label class="block text-xs text-gray-400 uppercase mb-1">Host</label>
              <input v-model="form.host" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="192.168.1.1" />
            </div>
            <div>
              <label class="block text-xs text-gray-400 uppercase mb-1">Port</label>
              <input v-model.number="form.port" type="number" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="22" />
            </div>
        </div>
        <div>
          <label class="block text-xs text-gray-400 uppercase mb-1">Username</label>
          <input v-model="form.username" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="root" />
        </div>
        <div>
        ...
        <div class="border-t border-gray-700 pt-4 mt-2">
            ...
            <div class="space-y-3 pl-2 border-l-2 border-gray-700">
                <div class="grid grid-cols-4 gap-4">
                    ...
                    <input v-model="form.jumpHost" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="jump.example.com" />
                    </div>
                    <div>
                    <label class="block text-xs text-gray-500 uppercase mb-1">Port</label>
                    <input v-model.number="form.jumpPort" type="number" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="22" />
                    </div>
                </div>
                <div>
                <label class="block text-xs text-gray-500 uppercase mb-1">Jump Username</label>
                <input v-model="form.jumpUsername" class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600 focus:border-blue-500 outline-none" placeholder="jumpuser" />
                </div>
                <div>
                ...
                <div class="relative">
                    <input 
                        ...
                        placeholder="••••••" 
                    />
                    <button @click="showJumpPassword = !showJumpPassword" class="absolute right-2 top-2 text-gray-400 hover:text-white">
      ...
      <div class="mt-6 flex justify-end space-x-2">
        <button @click="$emit('close')" class="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded cursor-pointer text-sm">Cancel</button>
        <button @click="save" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded cursor-pointer text-sm">Save</button>
      </div>
    </div>
  </div>
</template>
